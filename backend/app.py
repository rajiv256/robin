...
class OligoDesigner:
    ...
    def validate_sequence(self, sequence: str, settings: Dict) -> Dict:
        ...
        try:
            ...
            # Hairpin check
            try:
                hairpin_result = primer3.calc_hairpin(sequence, mv_conc=50, dv_conc=12.5,
            ...
            # Self-dimer check
            try:
                homodimer_result = primer3.calc_homodimer(sequence, mv_conc=50, dv_conc=12.5,
                                                          dntp_conc=0.6, dna_conc=50, temp_c=temp)
            ...
            # 3' hairpin check
            try:
                three_prime_hairpin_result = primer3.calc_hairpin(three_prime_end, mv_conc=50, dv_conc=12.5,
                                                                  dntp_conc=0.6, dna_conc=50, temp_c=temp)
    ...
    def calculate_cross_dimer_dg(self, seq1: str, seq2: str, temp: float = 37) -> float:
        ...
        try:
            heterodimer_result = primer3.calc_heterodimer(seq1, seq2, mv_conc=50, dv_conc=12.5,
                                                          dntp_conc=0.6, dna_conc=50, temp_c=temp)
    ...
    def calculate_three_prime_cross_dimer_dg(self, seq1: str, seq2: str, temp: float = 37) -> float:
        ...
        try:
            ...
            heterodimer_result = primer3.calc_heterodimer(three_prime_end, seq2, mv_conc=50, dv_conc=12.5,
                                                          dntp_conc=0.6, dna_conc=50, temp_c=temp)
            return heterodimer_result.dg / 1000.0
        except Exception:
            return 0.0
...
def should_skip_cross_dimer_check(strand1_domains: List[str], strand2_domains: List[str]) -> bool:
    """
    Check if cross-dimer analysis should be skipped between two strands.
    Skip if strands contain a domain and its reverse complement (designed to bind).

    Args:
        strand1_domains: List of domain names for strand 1
        strand2_domains: List of domain names for strand 2

    Returns:
        True if cross-dimer check should be skipped, False otherwise
    """
    # Extract base domain names from both strands
    strand1_bases = set()
    strand2_bases = set()

    for domain in strand1_domains:
        base_name = domain.rstrip('*')
        strand1_bases.add(base_name)

    for domain in strand2_domains:
        base_name = domain.rstrip('*')
        strand2_bases.add(base_name)

    # Check if any base domain appears in both strands
    # This indicates they share complementary domains and are designed to interact
    common_bases = strand1_bases.intersection(strand2_bases)

    if common_bases:
        return True  # Skip cross-dimer check - strands are designed to interact

    return False  # Perform cross-dimer check
    ...
    try:
        ...
        for i, strand1 in enumerate(target_strands):
            for j, strand2 in enumerate(target_strands):
                ...
                # Check if we should skip this cross-dimer check
                if should_skip_cross_dimer_check(strand1['domains'], strand2['domains']):
                    skipped_interactions.append({
                        'strand1': strand1['name'],
                        'strand2': strand2['name'],
                        'reason': 'Strands contain complementary domains (designed to interact)'
                    })
                    continue
        ...
        # Prepare message
        total_pairs = len(target_strands) * (len(target_strands) - 1)  # All directed pairs
        analyzed_pairs = len(results)
        skipped_pairs = len(skipped_interactions)

        message = f"Analyzed {analyzed_pairs} 3' end interactions ({len(target_strands)} strands). "
        message += f"Found {problematic_count} problematic interactions. "
        if skipped_pairs > 0:
            message += f"Skipped {skipped_pairs} interactions (complementary domains)."
...
def check_three_prime_analysis():
    ...
    try:
        ...
        for strand in target_strands:
            ...
            try:
                hairpin_result = primer3.calc_hairpin(three_prime_end, mv_conc=50, dv_conc=12.5,
                                                      dntp_conc=0.6, dna_conc=50, temp_c=temp)
            ...
            try:
                self_dimer_result = primer3.calc_heterodimer(three_prime_end, sequence, mv_conc=50, dv_conc=12.5,
                                                             dntp_conc=0.6, dna_conc=50, temp_c=temp)
            ...
            for other_strand in target_strands:
                if other_strand['name'] != strand['name']:
                    # Check if we should skip this cross-dimer check
                    if should_skip_cross_dimer_check(strand['domains'], other_strand['domains']):
                        cross_dimers.append({
        ...
        for strand_result in results:
            ...
            if strand_result['checks']['self_dimer']['problematic']:
                total_problematic += 1
            for cd in strand_result['checks']['cross_dimers']:
                if cd.get('skipped', False):
                    total_skipped += 1
                elif cd['problematic']:
                    total_problematic += 1
        ...
        if total_skipped > 0:
            message += f" Skipped {total_skipped} cross-dimer checks (complementary domains)."
...
def generate_optimized_strand_sets():
    ...
    try:
        ...
        for generation in range(num_generations):
            ...
            # Check cross-dimer interactions for this generation (with skipping logic)
            ...
            for i, strand1 in enumerate(generation_strands):
                for j, strand2 in enumerate(generation_strands):
                    ...
                    # Skip cross-dimer check if strands have complementary domains
                    if should_skip_cross_dimer_check(strand1['domains'], strand2['domains']):
                        continue
